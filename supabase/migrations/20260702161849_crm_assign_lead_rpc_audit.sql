-- Assignment contract: one transactional path for assignment/reassignment
-- plus a generic CRM audit log for operational changes.
...
create table if not exists public.crm_audit_events (
  ...
  constraint crm_audit_events_event_type_not_blank check (btrim(event_type) <> '')
);

create index if not exists crm_audit_events_lead_created_idx
  on public.crm_audit_events (lead_id, created_at desc);

create index if not exists crm_audit_events_entity_created_idx
  on public.crm_audit_events (crm_entity_id, created_at desc);
...
create policy crm_audit_events_select
...
create policy crm_audit_events_insert_authenticated
on public.crm_audit_events
for insert
to authenticated
with check (
...
create policy crm_audit_events_delete_admin
on public.crm_audit_events
for delete
to authenticated
