-- El reporte de actividad mide tiempos también con una campaña elegida.
--
-- Con el filtro "Equifax" todas las columnas de tiempo (conectado,
-- productivo, ocupación, jornada, disponible, pausa, colación) salían en
-- blanco: la función las anulaba al filtrar por campaña porque el estado del
-- ejecutivo no es por campaña. Pero la sesión del teléfono sí lo es
-- (dialer_agent_sessions[_history].campaign_id) y el turno también
-- (campaign_agent_schedules por membresía). Con campaña elegida: conectado y
-- productivo salen de la sesión de esa campaña; jornada, disponible, pausa y
-- colación, del estado del ejecutivo dentro del turno de esa campaña.
...
begin
  ...
  select p.id, p.full_name,
    ...
    round(coalesce(pa.logged, 0), 1),
    round(coalesce(pa.productive, 0), 1),
    round(100.0 * coalesce(pa.productive, 0) / nullif(pa.logged, 0), 1),
    case when sa.pid is null then null else round(sa.scheduled, 1) end,
    case when sa.pid is null then null else round(coalesce(ra.available, 0), 1) end,
    case when sa.pid is null then null else round(coalesce(ra.paused, 0), 1) end,
    case when sa.pid is null then null else round(coalesce(ra.disconnected, 0), 1) end,
    -- Adherencia contra el tiempo de trabajo del turno: las horas programadas
    -- menos la colación y el descanso planificados.
    case when sa.pid is null then null
      else round(least(100.0, 100.0 * coalesce(ra.available, 0) / nullif(sa.scheduled - coalesce(sa.planned_break, 0), 0)), 1)
    end,
    case when sa.pid is null then null else round(coalesce(sa.planned_break, 0), 1) end,
    case when sa.pid is null then null
  ...
  where public.can_access_org(p.organization_id) and p.role = 'agente'
    ...
    and (
      -- Con campaña: quien tuvo llamadas, sesión o turno en ella. El estado
      -- (Disponible/AUX) es del ejecutivo, no de la campaña: se mide dentro
      -- del turno de esa campaña.
      case when v_scoped then (ca.pid is not null or pa.pid is not null or sa.pid is not null)
