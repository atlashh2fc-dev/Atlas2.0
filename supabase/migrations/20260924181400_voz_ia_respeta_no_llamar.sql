-- La voz IA tampoco llama a la lista de no llamar.
--
-- El resguardo de dial_attempts (20260924181100) ya impide que salga el
-- intento, pero el claim de voz IA toma siempre los primeros leads por
-- created_at: un lead suprimido a la cabeza se volvería a elegir en cada ciclo
-- y la campaña quedaría detenida. Aquí se filtra antes de elegir. El resto de
-- la función queda igual que en 20260824214404.
--
-- Las agendas personales (claim_due_personal_callbacks) las redefine el frente
-- de agendas en 20260924182000; mientras tanto las cubre el resguardo, y como
-- solo reintentan dentro de su ventana no quedan trabadas.

create or replace function public.claim_next_ai_voice_targets(p_campaign_id uuid, p_batch_size integer default 1)
...
begin
  ...
  select config.max_concurrent_calls, config.max_attempts_per_contact, campaign.organization_id
  into v_max_concurrent, v_max_attempts, v_organization_id
