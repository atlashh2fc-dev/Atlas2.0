create table if not exists public.supervisor_report_daily_tipifications (
  ...
  primary key (metric_day, team_id, agent_id, label)
);

create index if not exists supervisor_report_tipifications_team_day_idx
  on public.supervisor_report_daily_tipifications (team_id, metric_day desc, count desc);

create index if not exists supervisor_report_tipifications_agent_day_idx
  on public.supervisor_report_daily_tipifications (agent_id, metric_day desc);

alter table public.supervisor_report_daily_tipifications enable row level security;

drop policy if exists supervisor_report_daily_tipifications_select on public.supervisor_report_daily_tipifications;
create policy supervisor_report_daily_tipifications_select
...
grant select on public.supervisor_report_daily_metrics to authenticated;
grant select on public.supervisor_report_daily_tipifications to authenticated;
revoke insert, update, delete on public.supervisor_report_daily_tipifications from anon, authenticated;

create or replace function public.refresh_supervisor_report_tipification_rows(
  p_day date,
  p_team_id uuid,
  p_agent_id uuid
)
...
create or replace function public.refresh_supervisor_report_tipifications_range(
  p_from date,
  p_to date,
...
declare
  v_row record;
begin
  delete from public.supervisor_report_daily_tipifications m
  ...
  loop
    perform public.refresh_supervisor_report_tipification_rows(v_row.metric_day::date, v_row.team_id, v_row.agent_id);
  end loop;
end;
$function$;
...
begin
  ...
  tipification_rows as (
    ...
    from public.supervisor_report_daily_tipifications t
...
select public.refresh_supervisor_report_tipifications_range(
  (current_date - interval '180 days')::date,
  current_date,
  null
);

revoke all on function public.refresh_supervisor_report_tipification_rows(date, uuid, uuid) from public, anon, authenticated;
revoke all on function public.refresh_supervisor_report_tipifications_range(date, date, uuid) from public, anon, authenticated;
