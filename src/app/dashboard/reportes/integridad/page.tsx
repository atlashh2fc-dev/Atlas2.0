...
import { Badge, Callout, Card, DataTable, MetricCard, type Column } from "@/components/ui";
...
const AGENT_COLUMNS: Column<IntegrityAgentRow>[] = [
  ...
  {
    id: "instantaneos",
    header: "Cierres instantáneos",
    align: "right",
    value: (row) => row.cierres_instantaneos,
  },
  {
    id: "sin_respaldo",
    header: "Contacto sin llamada",
    align: "right",
    value: (row) => row.contactos_sin_respaldo,
  },
...
const DETAIL_COLUMNS: Column<IntegrityDetailRow>[] = [
  {
    ...
    cell: (row) => formatDateTime(row.ended_at),
  },
  { id: "ejecutivo", header: "Ejecutivo", value: (row) => row.full_name },
  ...
  },
  {
    ...
    sortable: false,
    value: (row) =>
      [row.cierre_instantaneo && "instantáneo", row.contacto_sin_respaldo && "sin llamada", row.rafaga && "ráfaga"]
        .filter(Boolean)
        .join(", "),
    cell: (row) => (
    ...
    <div className="space-y-6">
      ...
      <Card>
        ...
        <p className="mb-3 text-xs text-muted-foreground">
          Hasta 500 gestiones, de la más reciente a la más antigua.
        </p>
        <DataTable
          rows={report.detail}
